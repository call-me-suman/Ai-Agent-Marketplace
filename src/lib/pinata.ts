...
export async function storeToIPFS(data: ChatInteraction): Promise<{ hash: string; url: string }> {
  try {
      ...
      PINATA_API_URL,
      {
        ...
        pinataMetadata: {
          ...
          keyvalues: {
            agentId: data.agentId,
            walletAddress: data.walletAddress,
            transactionType: data.transactionType || (data.transactionHash ? 'paid' : 'trial'),
...
export async function getHistoryFromIPFS(walletAddress: string): Promise<ChatInteraction[]> {
  try {
    const apiKey = process.env.NEXT_PUBLIC_PINATA_API_KEY;
    const secretKey = process.env.NEXT_PUBLIC_PINATA_SECRET_KEY;
    ...
    const historyPromises = queryResponse.data.rows.map(async (pin: any) => {
      try {
        const contentResponse = await axios.get(`${PINATA_GATEWAY_URL}/${pin.ipfs_pin_hash}`);
        ...
        console.error(`Failed to fetch content for hash ${pin.ipfs_pin_hash}:`, error);
        return null;
      }
    });
